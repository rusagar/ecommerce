...
Class Digital_Products extends Admin_Controller {
	...
	function index()
	{
                ...
                $this->template->set('title', 'Digital Products');
		$data['file_list']	= $this->digital_product_model->get_list();
		
		//$this->load->view('admin/digital_products', $data);
                $this->template->load('templates/admin/brainlight', 'admin/products/digital_products', $data);
	}
	...
	function form($id=0)
	{
		...
		if($id)
		{
			$data	= array_merge($data, (array)$this->digital_product_model->get_file_info($id));
                ...
                $this->template->set('title', 'Digital Product Products');
		
		$this->form_validation->set_rules('max_downloads', 'Max downloads', 'numeric');
		$this->form_validation->set_rules('title', 'Title', 'trim|required');

		
		if ($this->form_validation->run() == FALSE)
		{
			$this->template->load('templates/admin/brainlight', 'admin/products/digital_product_form', $data);
		} else {
			...
			if($id==0)
			{
				...
				if($this->upload->do_upload())
				{
					...
					//$this->load->view('admin/digital_product_form', $data);
                                        $this->template->load('templates/admin/brainlight', 'admin/products/digital_product_form', $data);
					return;
				}
