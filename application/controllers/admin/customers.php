...
class Customers extends Admin_Controller {
	...
	function index($field='lastname', $by='ASC', $page=0)
	{
		...
		$this->template->set('title', lang('customers'));
		$data['customers']	= $this->customer_model->get_customers(50,$page, $field, $by);
		
		$this->load->library('pagination');

		$config['base_url']		= base_url().'/'.'admin/customers/index/'.$field.'/'.$by.'/';
		...
		$data['by']	= $by;
       	...
       	$this->template->load('templates/admin/brainlight', 'admin/customers/index', $data);	
	}
	...
	function export_xml()
	{
		...
		force_download_content('customers.xml',	$this->template->load('templates/admin/blank', 'admin/customers/customers_xml', $data), true);
	...
	function addresses($id = false)
	{
		...
		$data['page_title']	= sprintf(lang('addresses_for'), $data['customer']->firstname.' '.$data['customer']->lastname);
                ...
                $this->template->load('templates/admin/brainlight', 'admin/customers/customer_addresses', $data);
	}
	...
	function order_list($status = false)
	{
       ...
       $this->template->load('templates/admin/brainlight', 'admin/order_list/index', $data);	
	}
	...
	function get_subscriber_list()
	{
		...
		$this->template->load('templates/admin/blank', 'admin/customers/customer_subscriber_list', $data);
	}	
