...
//		$this->columns = $this->config->item('columns', 'ion_auth');
                ...
                /**
                 * Copied from ion_auth library such that to make it un useful.
                 */
        }
        ...
        public function activate($id, $code = false)
        {
                if ($code !== false)
                {
                        ...
                        $this->db->where($this->ion_auth_model->_extra_where);
                ...
                else
                {
                        ...
                        $this->db->where($this->_extra_where);
         ...
         * @author Mathew
         * */
        public function register($username, $password, $email, $title, $first_name, $last_name, $additional_data = false, $group_name = false)
        {
                ...
                $data = array(
                    ...
                    'created_on' => now(),
                    'active' => 1,
                    //extra added to match need for bizelaw
                    'title' => $title,
                ...
                // so that we can fetch user id to deactivate with deactivation code
                $this->user_id = $id;
        ...
        public function login($identity, $password, $remember=FALSE)
        {
                ...
                $query = $this->db->select($this->identity_column . ', id, password, group_id, user_type')
                        ->where($this->identity_column, $identity)
                        ->where('active', 1)
                        ->where($this->_extra_where)
                        ->limit(1)
...
//            exit();
                ...
                if ($query->num_rows() == 1)
                {
                        ...
                        if ($result->password === $password)
                        {
                                ...
                                $group_row = $this->db->select('name, permissions')->where('id', $result->group_id)->get($this->tables['groups'])->row();
                                $meta_row = $this->db->where('user_id', $result->id)->get($this->tables['meta'])->row();
                                $session_data = array(
                                    ...
                                    'permissions' => $group_row->permissions,
                                    'user_type' => $result->user_type
                                );

                                // added this such that we have entity_id and is_admin is available when having recruiter login
                                if (isset($meta_row->entity_id))
                                {
                                        $session_data['is_admin'] = $meta_row->is_admin;
                                        $session_data['entity_id'] = $meta_row->entity_id;
                                }
...
//                    print_r($group_row);
//                    echo $group_row->permissions; exit();
//                    $_SESSION['permissions'] = $group_row->permissions;
         ...
         *
         * @return object Users
         * @author Ben Edmunds modified by Nitesh to suit Bizelaw's need
         * */
        public function get_users($limit=NULL, $offset=NULL)
        {
                ...
                if (isset($this->_meta_extra_where) && !empty($this->_meta_extra_where))
                {
                        $this->db->where($this->_meta_extra_where);
                }
                
                if (isset($this->_extra_where) && !empty($this->_extra_where))
                {
                        $this->db->where($this->_extra_where);
                }
        ...
        public function get_users_count($group=false)
        {
                ...
                // bizelaw specific change
                if (isset($this->_meta_extra_where) && !empty($this->_meta_extra_where))
                {
                        $this->db->join($this->tables['meta'], $this->tables['users'].'.id='.$this->tables['meta'].'.user_id','left');
                        $this->db->where($this->_meta_extra_where);
                }
        ...
        public function update_user($id, $data)
        {
                ...
                $this->db->where('id' , $id)->update($this->tables['users'], $data);
        ...
        public function login_remembered_user()
        {
                ...
                $query = $this->db->select($this->identity_column . ', id, group_id, user_type')
                ...
                if ($query->num_rows() == 1)
                {
                        ...
                        $meta_row = $this->db->where('user_id', $user->id)->get($this->tables['meta'])->row();
                        $group_row = $this->db->select('name, permissions')->where('id', $user->group_id)->get($this->tables['groups'])->row();
        ...
        //----------------------------------------------
        // These lines has been copied from ion_auth library, such that we can discard library as unuseful.
        /**
         * extra_where
         *
...
//                return $_output;
                ...
                foreach ($this->errors as $error)
                {
                        $error_msgs[] = $this->ci->lang->line($error);
                }
//                return $error_msgs;
                return $this->errors;
        }
